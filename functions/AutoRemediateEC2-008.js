...
module.exports.handler = (event, context, callback) => {
	...
	AccessRevoker.revoke(PROTOCOL, TCP_MSSQL_PORT, event.resource, event.region, function (
		err,
		result
	) {
		if (err) {
			...
			return handleError(
				err.message ? err.message : "removing mssql database access on port 1433 failed"
			);
		}
