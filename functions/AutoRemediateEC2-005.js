...
module.exports.handler = (event, context, callback) => {
	...
	AccessRevoker.revoke(PROTOCOL, TCP_MYSQL_PORT, event.resource, event.region, function (
		err,
		result
	) {
		if (err) {
			...
			return handleError(
				err.message ? err.message : "removing mysql database access on port 3306 failed"
			);
		}
