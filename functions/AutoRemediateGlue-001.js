...
module.exports.handler = (event, context, callback) => {
    console.log('Glue encryption not enabled, Received event: ', JSON.stringify(event, null, 2))
    if (!event || !event.region || event.ruleId !== 'Glue-001') {
        return handleError('Invalid event')
    }

    if(!CONFIG['AutoRemediateGlue-001']['topicName']) {
    ...
    var snsTopicArn = ['arn:aws:sns', event['region'], accountId, CONFIG['AutoRemediateGlue-001']['topicName']].join(":")
    var data = {}
