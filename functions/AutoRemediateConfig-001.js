...
module.exports.handler = (event, context, callback) => {
	...
	function getRole(accountId) {
		...
		return IAM.getRole({ RoleName: ConfigRoleName })
			.promise()
			.then(function (data) {
			...
			.catch(function (err) {
				...
				const CreateRoleParams = {
					...
					Description:
						"AWS Config Role Created By Cloud Conformity AutoRemediateConfig-001"
				};

				return IAM.createRole(CreateRoleParams)
					.promise()
					.then(function (data) {
						...
						return IAM.attachRolePolicy(AttachRolePolicyParams)
							.promise()
							.then(function () {
								console.log(
									"Successfully attach AWSConfigRole managed policy to role"
								);
										...
										Statement: [
											{
												...
												Resource:
													"arn:aws:s3:::" +
													CONFIG["AutoRemediateConfig-001"][
														"S3BucketName"
													] +
													"/AWSLogs/" +
													accountId +
								...
								return IAM.putRolePolicy(PutRolePolicyParams)
									.promise()
									.then(function () {
	...
	function subscribe(roleARN) {
		...
		return ConfigService.putConfigurationRecorder(ConfigurationRecorderParams)
			.promise()
			.then(function () {
				...
				return ConfigService.putDeliveryChannel(PutDeliveryChannelParams)
					.promise()
					.then(function () {
						...
						return ConfigService.startConfigurationRecorder(
							StartConfigurationRecorderparams
						)
							.promise()
							.then(function (data) {
